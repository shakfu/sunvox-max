...
int main()
{
    ...
    if (ver >= 0) {
        ...
        if (f) {
            void* buf = malloc(g_sv_buffer_size * g_sv_channels_num
                               * g_sv_sample_type); // Output audio buffer
            void* in_buf = malloc(g_sv_buffer_size * g_sv_channels_num
                                  * g_sv_sample_type); // Input audio buffer
            ...
            fwrite(&val, 4, 1, f);
            val = 1;
            if (g_sv_sample_type == 4)
                val = 3;
            fwrite(&val, 2, 1, f); // format
            val = g_sv_channels_num;
            fwrite(&val, 2, 1, f); // channels
            val = g_sv_sample_rate;
            fwrite(&val, 4, 1, f); // frames per second
            val = g_sv_sample_rate * g_sv_channels_num * g_sv_sample_type;
            fwrite(&val, 4, 1, f); // bytes per second
            val = g_sv_channels_num * g_sv_sample_type;
            fwrite(&val, 2, 1, f); // block align
            val = g_sv_sample_type * 8;
            fwrite(&val, 2, 1, f); // bits
            ...
            while (keep_running && cur_frame < out_frames) {
                ...
                sv_audio_callback2(
                    ...
                    g_sv_sample_type
                        == 4,          // input type: 0 - int16; 1 - float32
                ...
                // Print some info:
                int new_pos = (int)(((float)cur_frame / (float)out_frames)
                                    * 100);
                if (pos != new_pos) {
                    printf("%d %%\n", pos);
                    pos = new_pos;
