...
const int g_sv_channels_num = 2; // 1 - mono; 2 - stereo; only stereo supported
...
int main()
{
    ...
    if (ver >= 0) {
        ...
        if (f) {
            int frame_size = g_sv_channels_num
                * g_sv_sample_type; // bytes per frame
            ...
            val = g_sv_sample_rate * frame_size;
            ...
            while (keep_running && cur_frame < song_len_frames) {
                ...
                // Print some info:
                int new_pos = (int)(((float)cur_frame / (float)song_len_frames)
                                    * 100);
                if (pos != new_pos) {
