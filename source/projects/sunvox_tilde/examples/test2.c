...
int main()
{
    ...
    if (ver >= 0) {
        ...
        ); sv_unlock_slot( 0 ); float curve_data[ 1024 ]; int len =
        sv_module_curve( 0, multisynth, 1, curve_data, 0, 0 ); printf( "Curve
        ...
        */
        ...
        if (1) {
            ...
            if (data) {
                mod_num2 = sv_load_module_from_memory(
                    0, data, (unsigned int)size, 0, 0, 0);
                free(data);
            }
        }
        if (mod_num2 >= 0) {
